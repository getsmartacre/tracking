(function () {
	...
	function Referral(source, medium, term, content, campaign, adgroup, device, keyword) {
		this.source = source;
		this.medium = medium;
		this.term = term;
	...
	function getReferral(domain) {
		...
		var ref = document.referrer,
			...
			ha = getHash("utm_adgroup"),
			hd = getHash("utm_device"),
			hk = getHash("utm_keyword"),
			qs = getQuery("utm_source"),
			qm = getQuery("utm_medium"),
			qt = getQuery("utm_term"),
			qc = getQuery("utm_content"),
			qn = getQuery("utm_campaign"),
			qa = getQuery("utm_adgroup"),
			qd = getQuery("utm_device"),
			qk = getQuery("utm_keyword"),
			n;

		if (hs || hm || ht || hc || hn || ha || hd || hk) {
			...
			if (!hd) hd = "-";
			if (!hk) hk = "-";
			n = new Referral(hs, hm, ht, hc, hn, ha, hd, hk);

		} else if (qs || qm || qt || qc || qn || qa || qd || qk) {
			...
			if (!qd) qd = "-";
			if (!qk) qk = "-";
			n = new Referral(qs, qm, qt, qc, qn, qa, qd, qk);
	...
	function trackReferrals() {
		...
		if (tLast) {
			...
			setValue(tracker.ua, tLast["adgroup"]);
			setValue(tracker.ud, tLast["device"]);
			setValue(tracker.uk, tLast["keyword"]);
